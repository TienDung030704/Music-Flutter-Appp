...
class SongController
{
    ...
    public function search(?string $query): void
    {
        ...
        error_log("DEBUG SongController: iTunes API returned " . (is_array($results) ? count($results) : 'null') . " results");
        ...
        error_log("DEBUG SongController: Normalized to " . count($songs) . " songs");
        
        if (count($songs) > 0) {
            error_log("DEBUG SongController: First song: " . ($songs[0]['title'] ?? 'unknown'));
        }
    ...
    public function show(?string $id): void
    {
        ...
        Response::success(['song' => $this->normalizeSongDetail($detail)]);
    }
    ...
    {
        // Get top songs by searching for popular Vietnamese artists
        $popularArtists = [
            'Sơn Tùng MTP',
            'Đen Vâu', 
            'Hoàng Thuỳ Linh',
        ...
        foreach ($popularArtists as $artist) {
            ...
            if (is_array($results)) {
                $songs = $this->normalizeSearchResults($results);
                $topSongs = array_merge($topSongs, array_slice($songs, 0, 2));
            }
    ...
    private function normalizeSearchResults(array $items): array
    {
        ...
        foreach ($items as $item) {
            ...
            if (!$id || !$title || empty($item['previewUrl'])) {
                continue;
            }
            ...
            $results[] = [
                ...
                'thumbnail' => $this->formatArtwork($item['artworkUrl100'] ?? $item['artworkUrl60'] ?? null),
    ...
    private function normalizeSongDetail(array $detail): array
    {
        return [
            ...
            'title' => $detail['trackName'] ?? $detail['collectionName'] ?? null,
            'artists' => $detail['artistName'] ?? null,
            'album' => $detail['collectionName'] ?? null,
            'thumbnail' => $this->formatArtwork($detail['artworkUrl100'] ?? $detail['artworkUrl60'] ?? null),
    ...
    private function formatArtwork(?string $url): ?string
    {
        ...
        return preg_replace('/(\\d+)x(\\d+)(bb)?\\.([a-zA-Z]+)/', '512x512bb.$4', $url);
    }
}
